...
struct temp_transpose
{
  ...
  inline temp_gemm operator * (const matrix_handle&v_);
};
...
struct temp_scal
{
  ...
  inline temp_gemv operator * (const vector_handle&v_);
  inline temp_gemm operator * (const matrix_handle&v_);
  inline temp_gemm operator * (const temp_transpose&v_);
};

inline temp_gemm temp_scal::operator * (const temp_transpose&v_)
{
  return {a,trans,v_.trans,A,v_.A};
}

inline temp_gemv temp_scal::operator * (const vector_handle&v_)
{
  return {a,trans,A, v_};
};
inline temp_gemm temp_scal::operator * (const matrix_handle&v_)
{
  return {a, trans,'N',A, v_};
};

inline temp_scal temp_transpose::operator * (const R&v_)
{
  return {v_,trans,A};
};
inline temp_gemv temp_transpose::operator * (const vector_handle&v_)
{
  return {1.0,trans,A, v_};
};
  
inline temp_gemm temp_transpose::operator * (const matrix_handle&v_)
{
...
struct matrix_handle
{
  ...
  inline temp_gemv operator * (const vector_handle&v_);  
  inline temp_scal operator * (const R&v_);  
  inline temp_transpose transpose()
  {
    return {'T',*this};
  };
...
inline temp_gemm matrix_handle::operator * (const matrix_handle&v_)
  {
...
{
private:
  struct matrix_handle m_mat_tmpbrhs_uface0;
  struct matrix_handle m_mat_tmpbrhs_uface1;
  struct matrix_handle m_mat_tmpbrhs_uface2;
  ...
  struct matrix_handle m_u_eval;
  struct vector_handle m_hbrhs;
  struct matrix_handle m_hbmat;  
  struct matrix_handle m_mat_tmpbrhs_uelm;
  struct matrix_handle m_mat_brhs_uelm;  
  struct vector_handle m_brhs_u;
  struct vector_handle m_brhs_v;
public:  
...
public: void dgadvection_init_with_quadrature(cst_mkS 		s_teta_a_,
					      cst_mkS 		s_teta_u_,
					      cst_mkS 		s_teta_,
					      cst_mkS 		s_test_,
					      cst_mkS 		s_trial_,
     ...
     pR			rwork_)
  {
    ...
    I N_pts_u = bmat_m-na[0];/*nTot-nA*/
    
    pR tmpbrhs        	= &rwork_[bmat_m]; 
    pR tmpbrhs_uface0 	= &tmpbrhs[na[0]+dim*nu[0]]; 
    pR tmpbrhs_uface1 	= &tmpbrhs[na[0]+dim*nu[0]+qface_n_[0]];
    pR tmpbrhs_uface2 	= &tmpbrhs[na[0]+dim*nu[0]+2*qface_n_[0]];
    ...
    matrix_handle_def(&m_mat_tmpbrhs_uelm,
		      nu[0],
		      dim,
		      tmpbrhs_uelm,
		      tmpbrhs_ufaceoff);

    matrix_handle_def(&m_mat_brhs_uelm,
		      nu[0],
		      dim,
		      brhs_uelm,
		      nu[0]);

    vector_handle_def(&m_brhs_u,
		      N_pts_u,
		      tmpbrhs_u,
		      1);
    
    vector_handle_def(&m_brhs_v,
		      N_pts_u,
		      tmpbrhs_u + tmpbrhs_ufaceoff,
		      1);

    matrix_handle_def(&m_hbmat,
		      bmat_n,
		      bmat_m,(pR)bmat,
		      bmat_n);
    
    vector_handle_def(&m_hbrhs,
		      bmat_m,
		      brhs,
		      1);

    matrix_handle_def(&m_u_eval,
		      nu[0],
		      N_pts_u,
		      &rinfo_[iinfo_[DG::RA_EVAL_TETA_U]],
		      iinfo_[DG::I_TETA_U_NBASIS]);
    ...
    matrix_handle_def(&m_mat_tmpbrhs_uface0,qface_n_[0],2,tmpbrhs_uface0,tmpbrhs_ufaceoff);
    matrix_handle_def(&m_mat_tmpbrhs_uface1,qface_n_[0],2,tmpbrhs_uface1,tmpbrhs_ufaceoff);
    matrix_handle_def(&m_mat_tmpbrhs_uface2,qface_n_[0],2,tmpbrhs_uface2,tmpbrhs_ufaceoff);
  ...
  __state_next_elm:
    ...
    */  
    ...
    // Evaluate u.
    //
    m_brhs_u = m_u_eval.transpose() * u_ldofs;
    m_brhs_v = m_u_eval.transpose() * v_ldofs;
  
    /*____ TRANSFORM ON UELM */
    m_mat_brhs_uelm = mxu * m_mat_tmpbrhs_uelm * mat_belm.transpose();
    
    vec_uface0 = jacface[0] * m_mat_tmpbrhs_uface0 * vec_nrmelm0;
    vec_uface1 = jacface[1] * m_mat_tmpbrhs_uface1 * vec_nrmelm1;
    vec_uface2 = jacface[2] * m_mat_tmpbrhs_uface2 * vec_nrmelm2;
	...
	if (neids[localFaceIndex])
	  {
	    ...
	    hlcrhs += mat_matflux_corr * vec_neicorr;	 
	    hlcrhs += mat_matflux_residu * vec_neisol;	 
	  }
      }
