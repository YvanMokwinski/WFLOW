...
struct CG_VAR : public VAR<CG_VAR>
{
  ...
  void dofselm(I id, I icomp, pR dofs, I inc) const
  {
    ...
    for (I i=0;i<n;++i)
      {
	dofs[i*inc] = m_values.x[m_values.ld * icomp + m_mesh->cnc[6*id+i]-1];
      }
  };
  ...
  void setdofselm(I id, I icomp, cst_pR dofs, I inc) 
  {
    ...
    for (I i=0;i<n;++i)
      {
	m_values.x[m_values.ld * icomp + m_mesh->cnc[6*id+i]-1] = dofs[i*inc];
      }
  };
  ...
  CG_VAR(ns_mesh * mesh_, I ncomponents, mkS shape) : m_mesh(mesh_)
  {
    ...
    this->m_memory = (pR)calloc(this->m_ndofsPerComponent * this->m_ncomponents,sizeof(R));
    this->m_owner = true;
    matrix_handle_def(&this->m_values,this->m_ndofsPerComponent,this->m_ncomponents,this->m_memory,this->m_ndofsPerComponent);
  };
  ...
  CG_VAR(ns_mesh * mesh_,I ncomponents, mkS shape,pR mem_,pR memory_,I ld) : m_mesh(mesh_)
  {
    ...
    matrix_handle_def(&this->m_values,this->m_ndofsPerComponent,this->m_ncomponents,this->m_memory,ld);
  };
...
{  
  ...
  DG_VAR(ns_mesh * mesh_, mkS shape) : m_mesh(mesh_)
  {
    ...
    this->m_owner = true;
    matrix_handle_def(&this->m_values,mkS_n(&this->m_shape),mesh->nelm,this->m_memory,mkS_n(&this->m_shape));
  };
  ...
  DG_VAR(ns_mesh * mesh_,mkS shape,pR mem_,pR memory_,I ld) : m_mesh(mesh_)
  {
    ...
    this->m_owner = false;
    this->m_memory = memory_;
    matrix_handle_def(&this->m_values,mkS_n(&this->m_shape),this->m_mesh->nelm,this->m_memory,ld);
  };
  ...
  void dofselm(I id, I icomp, pR dofs, I inc) const
  {
    ...
    for (I i=0;i<n;++i)
      {
	dofs[i*inc] = m_values.x[m_values.ld * id + i];
      }
  };
  ...
  void setdofselm(I id, I icomp, cst_pR dofs, I inc) 
  {
    ...
    for (I i=0;i<n;++i)
      {
	m_values.x[m_values.ld * id + i] = dofs[i*inc];
      }
  };
...
p[9] =     0.9739065285171717200779640;
  ...
  {
    //    vector_handle_print(&f,stdout);
    //    matrix_handle_print(&this->m_beval_f,stdout);
    //    exit(1);
    this->m_f = this->m_beval_f.transpose() * f;
    //    vector_handle_print(&m_f,stdout);
...
#define JJ (*jacobians[jacdeg])
			       ...
			       pR  		rwork_)
  {
	...
	*/  
	...
	for (I localFaceIndex=0;localFaceIndex<nfaceinelm;++localFaceIndex)
	  {
		...
		vector_handle_def(&data_->vec_neisol,teta_n[0],   (pR)&sol_.m_values.x[(neids[localFaceIndex]-1)*sol_.m_values.ld], 1);
		data_->m_local_rhs -= data_->m_flux_part_sol  * data_->vec_neisol;	 
	      }
	...
	{
	  data_->m_local_matrices = handle_->m_BMAT * handle_->m_BRHS;
	  vector_handle_def(&data_->hsol,teta_n[0],(pR)&sol_.m_values.x[id*sol_.m_values.ld],1);
	  data_->m_local_rhs -= data_->m_local_matrix_part_sol * data_->hsol;
	}
	...
	for (int jadj=0;jadj<3;++jadj)
	  {
		...
		for (I k=0;k<m_ntest;++k)
		  {
		    residual_.m_values.x[jelm * residual_.m_values.ld + k] += data->lrhs.x[k];
		  }		
	      }
	  }
      }
...
I jj{};
void compute_nrm(DG_JACOBIAN*jacobian,I degree,pR 		x_)
{
  {
    I n1 = 1;
	 ...
	 const char**	argv)
{
  ...
  dg[degree-1] = new DG_OPERATOR(&shape_X,
				 &shape_A,				
				 &shape_FF[degree-1],
				 &shape_EE[degree-1],
				 &shape_EE[degree-1],
  ...
  auto aaa = new DG_OPERATOR(&shape_X,
			     &shape_A,				
			     &shape_FF[degree],
			     &shape_EE[degree-1],
			     &shape_EE[degree-1],
...
#endif
  //
  // Ce n'est pas le residu de la condition au bord.
  //
#if 0
  {
    pSparse matrix = Sparse_build(dg[degree]->m_jacobian->m_n,
  ...
  I solve_method=2;
    if (solve_method==1)
      {
  solve_gauss_seidel(*dg[degree-1]->m_jacobian,
		     200,
		     degree-1,
		     Correction0.m_memory,
		     Residual0.m_memory);

  DG_VAR Residual0(mesh, &shape_EE[degree-1]);
  DG_VAR F0(mesh, &shape_EE[degree-1]);
  ...
  dg[degree-1]->m_bc->boundary_condition(U,
					 Residual0);
  for (I i=0;i<mesh->nelm;++i)
    {
      F0.m_memory[i] = F.m_memory[3*i+0];
    }
  dg[degree-1]->compute_residual(F0,
				 U,
				 Residual0,
				 &dg[degree-1]->m_dginfo->dg_rwork_n,
				 dg[degree-1]->m_dginfo->dg_rwork);
  ...
  for (I i=0;i<mesh->nelm;++i)
    {
      F.m_memory[3*i+0] += Correction0.m_memory[i];
    }
  ...
  for (I i=0;i<mesh->nelm;++i)
    {
      F.m_memory[3*i+0] += E.m_memory[3*i+0];
      F.m_memory[3*i+1] += E.m_memory[3*i+1];
      F.m_memory[3*i+2] += E.m_memory[3*i+2];
    }
  ...
  for (I iter=0;iter < -1;++iter)
    {

  for (I i=0;i<3*mesh->nelm;++i) Residual.m_memory[i]=0.0;
  for (I i=0;i<mesh->nelm;++i) Residual0.m_memory[i]=0.0;
  for (I i=0;i<3*mesh->nelm;++i) E.m_memory[i]=0.0;
  for (I i=0;i<mesh->nelm;++i) Correction0.m_memory[i]=0.0;
  
  dg[degree]->m_bc->boundary_condition(U,
				       Residual);
  //  matrix_handle_print(&Residual.m_values,stdout);
  ...
  dg_print_sol	(view,
		 ...
		 F.m_memory,
		 "dgjacobi.F." ifmt,degree);
	daxpy(&N,&r1,E.m_memory,&n1,F.m_memory,&n1);
  dg_print_mesh( mesh,
		 view,
		 "dgjacobi.F2." ifmt,degree);  
  ...
  exit(1);
	...
	//	R r1=1.0;
#if 0
	aaa->m_bc->boundary_condition(U,
					       Residual0);
	aaa->compute_residual(F,
			      U,
			      Residual0,
			      &aaa->m_dginfo->dg_rwork_n,
			      aaa->m_dginfo->dg_rwork);
#endif
	R mr1=-1.0;
	I N2= mesh->nelm;
	//	dscal(&N2,&mr1,Residual0.m_memory,&n1);
#if 1
	dg[degree-1]->m_bc->boundary_condition(U,
					       Residual0);
	for (I i=0;i<mesh->nelm;++i)
