...
void Fem_def_from_quadrature(pFem const 	fem_,
			     cst_pI			qn_,
			     cst_pR 			qw_,
			     cst_pR 			qp_,
			     cst_pI			qoff_,
...
#define define_obasis(_a,_v) obasis_(_a,qn_,qp_,qoff_,fem_->shape##_v,&n##_v); \
  ...
  { I i,j,k;
	...
	for (j=0;j<_ndg;++j)
	  {
		...
		nsblas_dger(&nu,&nu,&a,&fem_->drshapeu[i*_nu],&negal1,&fem_->drshapeu[i*_nu],&negal1,&fem_->qref_radius_visc_dru_dru[_ndg*nuxnu*k+j*_nuxnu],&nu);
		nsblas_dger(&nu,&nu,&a,&fem_->dsshapeu[i*_nu],&negal1,&fem_->dsshapeu[i*_nu],&negal1,&fem_->qref_radius_visc_dsu_dsu[_ndg*nuxnu*k+j*_nuxnu],&nu); 	
		nsblas_dger(&nu,&nu,&a,&fem_->dsshapeu[i*_nu],&negal1,&fem_->drshapeu[i*_nu],&negal1,&fem_->qref_radius_visc_dru_dsu[_ndg*nuxnu*k+j*_nuxnu],&nu); 	
		nsblas_dger(&nu,&nu,&a,&fem_->drshapeu[i*_nu],&negal1,&fem_->dsshapeu[i*_nu],&negal1,&fem_->qref_radius_visc_dsu_dru[_ndg*nuxnu*k+j*_nuxnu],&nu); 	  
	      }
	  }        	
	for (j=0;j<_ndg;++j)
	  {
	    for (i=0;i<qn_[0];++i)
	      {
		const R a = qw_[i] * fem_->shapedg[i*_ndg+j] *fem_->shaper[i*3+k];
		nsblas_dger(&nu,&nu,&a,&fem_->shapeu[i*_nu],&negal1,&fem_->shapeu[i*_nu],&negal1,&fem_->qref_radius_dens_u_u[_ndg*nuxnu*k+j*_nuxnu],&nu);
	      }
	  }	
	for (i=0;i<qn_[0];++i)
	  {
	    const R a = qw_[i] * fem_->shaper[i*3+k];
	    nsblas_dger(&np,&nu,&a,&fem_->shapep[i*_np],&negal1,&fem_->drshapeu[i*_nu],&negal1,&fem_->qref_radius_dru_p[k*_nu*_np],&np);
	    nsblas_dger(&np,&nu,&a,&fem_->shapep[i*_np],&negal1,&fem_->dsshapeu[i*_nu],&negal1,&fem_->qref_radius_dsu_p[k*_nu*_np],&np); 	
	    nsblas_dger(&nu,&np,&a,&fem_->drshapeu[i*_nu],&negal1,&fem_->shapep[i*_np],&negal1,&fem_->qref_radius_p_dru[k*_nu*_np],&nu);
	    nsblas_dger(&nu,&np,&a,&fem_->dsshapeu[i*_nu],&negal1,&fem_->shapep[i*_np],&negal1,&fem_->qref_radius_p_dsu[k*_nu*_np],&nu); 	
	  }  
      } }
    ...
    void Fem_def(pFem const fem_,cst_eFace element_ ,STR errmsg_,Err*err_)
    {
      err_[0] = __eErr_no;
  I dec = 0;
  I n;
  ...
  n = _nu*_ndg*_ndg;memcpy(&fem_->build_twice_partialx[_build_twice_partialoff*3+dec],exactref_dsf_u_dsf,sizeof(R)*n);dec+=n;
    ...
    void Fem_setelm(pFem const 	fem_,
		       pWorkelm const 	elm_)
{
...
#if 0
  printf("alllo %e  %e %e %e",sbelm2[0],sbelm2[1],sbelm2[2],sbelm2[3]);
#endif
  dgemm	(transN,
		 transN,
		 ...
		 &build_twice_partialoff);    
#if 0
  for (int i=0;i<10;++i) printf("ggg %p %p %e\n",fem_->build_twice_partialr,fem_->exact_visc_dxu_dxu,fem_->build_twice_partialr[i]);
#endif
}
