...
#ifndef NDEBUG
#if 0
cst_pR 	nsSPACE_mass		(pSpaceReadOnly const S_,cst_ensBASIS shape_) 	{ return S_->MASS[shape_];}
cst_pR 	nsSPACE_convx		(pSpaceReadOnly const S_,cst_ensBASIS shape_) 	{ return S_->CONV_DR[shape_];}
cst_pR 	nsSPACE_convy		(pSpaceReadOnly const S_,cst_ensBASIS shape_) 	{ return S_->CONV_DS[shape_];}
#endif
cst_eFace 	nsSPACE_elm		(pSpaceReadOnly const S_) 				{ return ns_mesh_elm(S_->m_grid); }
const ns_mesh * nsSPACE_mesh		(pSpaceReadOnly const S_) 				{ return S_->m_grid; }
I 		nsSPACE_nddl		(pSpaceReadOnly const S_) 				{ return S_->nddl; }
I 		nsSPACE_nddlelm		(pSpaceReadOnly const S_) 				{ return S_->nddlelm; }
I 		nsSPACE_nddledge	(pSpaceReadOnly const S_) 				{ return S_->nddledge; }
ensBASIS 	nsSPACE_shape		(pSpaceReadOnly const S_) 				{ return S_->shape; }
cst_pR 		nsSPACE_ddlcoo		(pSpaceReadOnly const S_) 				{ return S_->ddlcoo; }
cst_pI 		nsSPACE_ddlcnc		(pSpaceReadOnly const S_) 				{ return S_->ddlcnc; }
cst_pI 		nsSPACE_ddlcod		(pSpaceReadOnly const S_) 				{ return S_->ddlcod; }
#endif

pSparse nsSPACE_get_endomorphism(pSpaceReadOnly const S_)
{

  pSparse back  = Sparse_buildReference(Sparse_get_n(S_->m_endomorphism),
					Sparse_get_m(S_->m_endomorphism),
					Sparse_get_nc(S_->m_endomorphism),
					Sparse_get_b(S_->m_endomorphism),
					Sparse_get_i(S_->m_endomorphism),
					NULL);
  return back;
#if 0
  nsSPARSE_ST * F = malloc(sizeof(nsSPARSE_ST));
  memcpy(F,&S_->m_endomorphism,sizeof(nsSPARSE_ST));
...
pSparse ns_mesh_get_endomorphism	(ns_mesh * 			mesh_,
					 cst_ensBASIS 	shape_)
{
  return nsSPACE_get_endomorphism( ( mesh_->spaces[shape_] ) ? (pSpaceReadOnly)mesh_->spaces[shape_] : nsSPACE_new(mesh_,shape_) );
}

I 		nsSPACE_iddl2ddlP6	(pSpaceReadOnly 		SF_,
					 const I 		iddl_)
{
  const I numGridNodes = ns_mesh_get_numNodes(SF_->m_grid);
  const I numGridEdges = ns_mesh_get_numEdges(SF_->m_grid);

  if (iddl_<numGridNodes)
    {
      return iddl_;
    }
  else if (iddl_<numGridNodes+numGridEdges*SF_->nddledge)
    {
      return numGridNodes + (iddl_-numGridNodes)/SF_->nddledge;
    }
  else
    {
#if 0
      printf("iddl_ = "ifmt"\n",iddl_);
      printf("elm = "ifmt"\n",(iddl_-numGridNodes-numGridEdges*SF_->nddledge)/(SF_->nddlelm-SF_->nddledge*3-3));
#endif
      return numGridNodes + numGridEdges + (iddl_-numGridNodes-numGridEdges*SF_->nddledge)/(SF_->nddlelm-SF_->nddledge*3-3);
    }
}


static I nsSPACE_galerkin_line_fups( pSpaceReadOnly 	SF_,
					 pSpaceReadOnly 	SU_,
					 pSpaceReadOnly 	SP_,
					 pSpaceReadOnly 	SS_,
					 const I 		iddl_,
					 ...
					 cst_pI		total_nddl_)
{
  ...
  { I i;
    ...
    for (i=0;i<n;++i)
      {	
	const I jelm = (jddl>=numGridNodes+numGridEdges) ? jddl-(numGridNodes+numGridEdges) : mesh->patch[mesh->bpatch[jddl]+i];	
...
static I nsSPACE_galerkin_line_p( pSpaceReadOnly 	SU_,
				       pSpaceReadOnly 	SP_,
				       const I 		iddl_,
				       pI 			ddl_,
				       pI			blank_,
...
static I nsSPACE_galerkin_line( pSpaceReadOnly	S_,
				     const I 	iddl_,
				     pI 		ddl_,
				     pI		blank_,
				     pI 		symbolic_cnc_)
...
static I nsSPACE_galerkin_line_s( pSpaceReadOnly SU_,
				       pSpaceReadOnly SS_,
				       const I 	iddl_,
				       pI 		ddl_,
				       pI		blank_,
...
static void nsSPACE_galerkin_selectdiscbis	(pSpaceReadOnly 	SF_,
						 pSpaceReadOnly 	SU_,
						 const I 	jelm,
						 const I 	n_,
						 pI 		iwork_n_,
...
static void nsSPACE_galerkin_selectdisc	(pSpaceReadOnly 	SF_,
						 pSpaceReadOnly 	SU_,
						 cst_pI 		jelm_,
						 const I 		n_,
						 pI 			iwork_n_,
...
void nsSPACE_divergence(pSpaceReadOnly 	SU_,
			pSpaceReadOnly 	SP_,
			cst_pI 		axi_,
...
pSparse nsSPACE_galerkin_stokes_def(pSpaceReadOnly 	SF_,
				    pSpaceReadOnly 	SU_,
				    pSpaceReadOnly 	SP_,
				    pSpaceReadOnly 	SS_,
				    ...
				    pI		nddl_slip_)
{
  ...
  if (SS_)
    {
      ...
      Ai = (pI)malloc(total_nddlelm*total_nddlelm*mesh->nelm+2*mesh->nedge_boundary*(12+1)*sizeof(I));     /*+1 pour l'element diagonal du slip */
    }
  else
    {
      Ai = (pI)malloc((total_nddlelm+1)*(total_nddlelm+1)*mesh->nelm*sizeof(I));
      printf("size "ifmt"\n",(total_nddlelm+1)*(total_nddlelm+1)*mesh->nelm);
    }


  pI  blank = (pI)calloc(An,sizeof(I));
  ...
  if (SF_)
	...
	{
	  pI iwork = (pI)malloc(sizeof(I)*An);
	  { I jelm;
...
#if 0
  A_->own_x    	= (pR)malloc(Ancoeff*sizeof(R));
  A_->own_b    	= Ab;
  A_->b 	= A_->own_b;
  A_->x 	= A_->own_x;
  A_->i 	= A_->own_i;
...
Err nsSPARSE_STOKES_def_eulerian(nsSPARSE_STOKES_ST*const 	S_,
				      pSpaceReadOnly const 		SF_,
				      pSpaceReadOnly const 		SU_,
				      pSpaceReadOnly const 		SP_,
				      pSpaceReadOnly const 		SS_,
				      const L  		F_continuity_,
				      cst_pI 			axi_,
				      pI			slip_iperm_,
				      pI			nddl_slip_)
...
pSparse nsSPACE_galerkin_new(pSpaceReadOnly	S_)
{
  ...
  pI own_i	= (pI)malloc(S_->nddlelm*S_->nddlelm*mesh_->nelm*sizeof(I));
  own_b[0] 	= 0;
  I nc 		= 0;
#if 0
  F_->n		= S_->nddl;
  F_->m    	= S_->nddl;
  F_->own_b	= (pI)malloc((F_->n+1)*sizeof(I));
  F_->own_i	= (pI)malloc(S_->nddlelm*S_->nddlelm*mesh_->nelm*sizeof(I));
  F_->own_b[0] 	= 0;
  F_->nc 	= 0;
#endif
  ...
  I symbolic_cnc[128];
  I	nline;
  pI blank = (pI)calloc(S_->nddl,sizeof(I));
  { I iddl;
...
#if 0
void nsSPACE_galerkin_add_mass_ns_var(pSpaceReadOnly 	S_,
				       nsSPARSE_ST*		A_,
				       const ns_var* 		v_,
				       nsCUBA_TRIA_ST * 	c,
				       cst_pI 			rwork_n_,
...
pSpaceReadOnly ns_mesh_get_space(const ns_mesh*mesh_,
				 cst_ensBASIS shape_)
{
  return (pSpaceReadOnly)mesh_->spaces[shape_];
}

pSpaceReadOnly ns_mesh_addspace	(ns_mesh*mesh_,
				 cst_ensBASIS shape_)
{
...
void  nsSPACE_get_ddlcoo(pSpaceReadOnly	s_,
			 ...
			 cst_pI 		posoff_)
{
  const I dim = ns_mesh_dimension(s_->m_grid);
  { I i;
    for (i=0;i<nddl_[0];++i)
      {
	{ I j;
...
void nsSPACE_write_medit(pSpaceReadOnly s_,const char * name_,...)
{
  { 
    char ctmp[512];
      ...
      for (i=0;i<nelm;++i)
	{
	  ...
	  { I j;    
	    for (j=0;j<s_->nddlelm;++j)
	      {
		cncelm[j] = s_->m_grid->cnc[i*s_->ddlcncoff+j];		
	      } }
...
void nsSPACE_dg_write_medit(pSpaceReadOnly s_,const char * name_,...)
{
  char ctmp[512];
  const ns_mesh*mesh= nsSPACE_mesh(s_);
...
#endif
      s_->m_endomorphism = Sparse_kill(s_->m_endomorphism);
      memset(s_,0,sizeof(Space));
    }
}

pSpace	nsSPACE_kill	(pSpace  s_)
{
  if (s_)
    {
      nsSPACE_free(s_);
				 ...
				 cst_ensBASIS 	shape_)
{
  ...
  const I numGridNodes = ns_mesh_get_numNodes(mesh_);
  const I numGridEdges = ns_mesh_get_numEdges(mesh_);

  switch(space_->shape)
    {
    case __ensBASIS_LAGRANGEBUBBLE_0:
      {
	space_->own_ddlcnc 	= (pI)malloc(sizeof(I)*mesh_->nelm*2);
    ...
    case __ensBASIS_LAGRANGE_0:
      {
	space_->own_ddlcnc 	= (pI)malloc(sizeof(I)*mesh_->nelm);
	space_->ddlcnc 		= space_->own_ddlcnc;
	space_->ddlcncoff 	= 1;
	{ I i;
	  for (i=0;i<mesh_->nelm;++i)
    ...
    case __ensBASIS_LAGRANGEBUBBLE_2:
      {
	space_->own_ddlcnc 	= (pI)malloc(sizeof(I)*mesh_->nelm*space_->nddlelm);
	space_->ddlcnc 		= space_->own_ddlcnc;
	  ...
	  for (i=0;i<numGridEdges;++i)
		...
		{ I j;
		  for (j=0;j<space_->nddledge;++j)
		    {
		      space_->own_ddlcnc[space_->nddlelm*ielm+3+space_->nddledge*ilocedge+j] = numGridNodes + space_->nddledge*i + j+1;
		    } }
		if (nadj>1)
		  {
		    { I j;
		      for (j=0;j<space_->nddledge;++j)
			{
			  space_->own_ddlcnc[space_->nddlelm*jelm+3+space_->nddledge*jlocedge+space_->nddledge-1-j] = numGridNodes + space_->nddledge*i + j+1;
			} }
		  } 				
	      }
	    } }
	{ I i;
	  for (i=0;i<mesh_->nelm;++i)
		...
		for (j=0;j<n;++j)
		  {
		    space_->own_ddlcnc[space_->nddlelm*i+3+space_->nddledge*3+j] = numGridNodes+space_->nddledge*numGridEdges+n*i+j+1;
		  } }
	    } }
	break;
      }
  ...
  switch(space_->shape)
    ...
    case __ensBASIS_LAGRANGEBUBBLE_2:
      {
	space_->own_ddlcoo 	= (pR)malloc(sizeof(R)*space_->nddl*2);
    ...
    case __ensBASIS_L2ORTHONORMAL_0:
      {
	space_->own_ddlcoo 	= (pR)malloc(sizeof(R)*mesh_->nelm*2);
	space_->ddlcoo 		= space_->own_ddlcoo;
	{ I i;
	  R 	cooelm	[32];
  ...
  switch(space_->shape)
    ...
    default:
      {
	space_->own_ddlcod 	= (pI)malloc(sizeof(I)*space_->nddl);
  ...
  switch(space_->shape)
    ...
    case __ensBASIS_LAGRANGEBUBBLE_2:
      {
	{ I i;
	  for (i=0;i<numGridNodes+numGridEdges;++i)
	    {
	...
	{ I i;
	  for (i=0;i<mesh_->nelm;++i)
	    {
	      space_->own_ddlcod[numGridNodes+numGridEdges+i]	= mesh_->noboundary_vcod;
	    } }
	...
	{ I i;
	  for (i=0;i<numGridEdges;++i)
		...
		for (j=0;j<2;++j)
		  {
		    space_->own_ddlcod[numGridNodes+2*i+j] = mesh_->cod[numGridNodes+i];
		  } }
	    } }
	{ I i;
	  for (i=numGridNodes+2*numGridEdges;i<space_->nddl;++i)
	    {
	      space_->own_ddlcod[i] = mesh_->noboundary_vcod;
	    } }	
	break;
  ...
  { ens_lagr_tria j 		= __ens_lagr_tria_error;
	...
	S_->MASS 	[j] 	= (pR)calloc(nj*n*n,sizeof(R));
	S_->CONV_DR 	[j] 	= (pR)calloc(nj*n*n,sizeof(R));
	S_->CONV_DS 	[j] 	= (pR)calloc(nj*n*n,sizeof(R));
	{ I k;
	  for (k=0;k<nj;++k)
	    {
	      { I i;
...
pSpace nsSPACE_new 	(const ns_mesh * 	mesh_,
				 cst_ensBASIS 	shape_)
{
  pSpace space =(pSpace)malloc(sizeof(Space));
  nsSPACE_def 	(space,  
		 mesh_,
		 shape_);
  return space;
...
void nsSPACE_elm2ddl(pSpaceReadOnly 	space_,
		      cst_pI		select_n_,
		      cst_pI		select_,
		      pI       	ddl_,
		      pI   		ddl_n_,
...
void nsSPACE_elm2ddl_hat(pSpaceReadOnly 	space_,
			  cst_pI		ivertex_,
			  cst_pI		select_n_,
			  cst_pI		select_,
			  pI       		ddl_,
...
void nsSPACE_patch_vertices2elm(pSpaceReadOnly 	space_,
				 cst_pI 		level_,
				 cst_pI 		nvertices_,
				 cst_pI 		vertices_,
				 pI 			select_n_,
...
void	nsSPACE_patch_level_vertices(pSpaceReadOnly	space_,
				      cst_pI 		level_,					      
				      pI		select_n_,
				      pI		select_,
				      pI		blank_)
...
void nsSPACE_infopatch_hat(pSpaceReadOnly	space_,
			    cst_pI 	ivertex_,
			    pI 	patchvertex_n_,
			    pI 	patchvertex_,
			    pI 	patchelm_n_,
...
#if 0
nsSPARSE_ST*nsSPACE_smooth_matrix(pSpaceReadOnly space_)
{
...
#if 0
static cst_pR nsSPACE_K_M	(pSpaceReadOnly 	S_,
				 const ens_lagr_tria 	coeff_shape_)
{
  return S_->MASS[coeff_shape_];
}

static cst_pR nsSPACE_K_CDR	(pSpaceReadOnly 	S_,
				 const ens_lagr_tria 	coeff_shape_)
{
  return S_->CONV_DR[coeff_shape_];
}

static cst_pR nsSPACE_K_CDS	(pSpaceReadOnly 	S_,
				 const ens_lagr_tria 	coeff_shape_)
{
  return S_->CONV_DS[coeff_shape_];
}

static void nsSPACE_K_CDR_matrix(pSpaceReadOnly 	space_,
				  const ens_lagr_tria 	coeff_shape_,
				  cst_pR 		xcoeff_,
				  cst_pR		coeff_,
				  cst_pI		coeffoff_,
...
static void nsSPACE_K_CDS_matrix(pSpaceReadOnly 		space_,
			   const ens_lagr_tria 		coeff_shape_,
			   cst_pR 			xcoeff_,
			   cst_pR			coeff_,
			   cst_pI			coeffoff_,
...
void nsSPACE_K_M_matrix(pSpaceReadOnly 		space_,
			 const ens_lagr_tria 		coeff_shape_,
			 cst_pR 			xcoeff_,
			 cst_pR			coeff_,
			 cst_pI			coeffoff_,
