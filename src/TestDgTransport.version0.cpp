...
void  dgadvection_init_with_quadrature(cst_mkS 		s_teta_a_,
				       ...
				       pR		rwork_)
{  
  pI ferr_ 	= &iinfo_[DG_ERR];  
  ferr_[0] 	= (I)0;  
  if (iinfo_[DG_I_QFACE_N]<1)
    {
      ferr_[0] =
	DGERR_USER;
      fprintf(stderr,"*** MOK_DGI_INIT ERROR IINFO[DG_I_QFACE_N]<1\n");
      exit(1);
    }
  
  if (iinfo_[DG_I_QELM_N]<1)
    {
      ferr_[0] = DGERR_USER;
      fprintf(stderr,"*** MOK_DGI_INIT ERROR IINFO[DG_I_QELM_N]<1\n");
      exit(1);
    }
  
  iinfo_[DG_I_TETA_A_NBASIS]    = (s_teta_a_) ? mkS_n(s_teta_a_):((I)0);
  iinfo_[DG_I_TETA_U_NBASIS]	= mkS_n(s_teta_u_);
  iinfo_[DG_I_TETA_NBASIS]	= mkS_n(s_teta_);
  iinfo_[DG_I_TRIAL_NBASIS]	= mkS_n(s_trial_);
  iinfo_[DG_I_TEST_NBASIS]	= mkS_n(s_test_);
  if (ferr_[0]) 
    {
      return;
    }
  
  if (iinfo_[DG_I_TRIAL_NBASIS]!=iinfo_[DG_I_TEST_NBASIS])   
    { 
      ferr_[0] = DGERR_USER;
      fprintf(stderr,"*** MOK_DGI_INIT ERROR IINFO[DG_I_TEST_NBASIS]!=IINFO[DG_I_TRIAL_NBASIS]\n");    
    }

  iinfo_[DG_I_QELM_N] = qelm_n_[0];  
  ...
  const I trial_n	= iinfo_[DG_I_TRIAL_NBASIS];
  const I teta_n	= iinfo_[DG_I_TETA_NBASIS];
  const I nA	 	= iinfo_[DG_I_TETA_A_NBASIS];
  const I nU	 	= iinfo_[DG_I_TETA_U_NBASIS];
  ...
  iinfo_[DG_I_nTot]     	= numValues;
  iinfo_[DG_I_nTotElm]  	= numValuesOnElement;
  iinfo_[DG_I_nTotFace] 	= numValuesOnFace;
  
  /* matrice de flux */  
  /* les points d evaluation en coordonnees locales */
  iinfo_[DG_IA_lc]        	= 0;
  iinfo_[DG_IA_lc_elm]    	= 0;
  iinfo_[DG_IA_lc_elm_a]  	= 0;
  iinfo_[DG_IA_lc_elm_u0] 	= iinfo_[DG_I_TETA_A_NBASIS];
  iinfo_[DG_IA_lc_elm_u1] 	= iinfo_[DG_I_TETA_A_NBASIS]+iinfo_[DG_I_TETA_U_NBASIS];
  iinfo_[DG_IA_lc_face]   	= numValuesOnElement;
  iinfo_[DG_IA_lc_u]      	= iinfo_[DG_I_TETA_A_NBASIS];

  iinfo_[DG_I_bmat_n] 		= bmat_n;
  iinfo_[DG_I_bmat_m] 		= numValues;
  iinfo_[DG_I_bmat_len] 	= bmat_n*numValues;
  iinfo_[DG_RA_bmat] 		= 2*numValues;
  iinfo_[DG_RA_bmatx] 		= iinfo_[DG_RA_bmat]  + iinfo_[DG_I_bmat_len];
  iinfo_[DG_RA_EVAL_TETA_U]     = iinfo_[DG_RA_bmatx] + bmatx_size;

  iinfo_[DG_I_lc_len] 		= 2*numValues;
  
  const I npts_velocity 	= iinfo_[DG_I_nTot]-iinfo_[DG_I_TETA_A_NBASIS];  
    
  pR bmat_x 			= &rinfo_[iinfo_[DG_RA_bmat]];  
  if (nA>0)
    {
  ...
  /* LOCAL COORDINATES -------------------------------------------------------------------  */
  /* -------------------------------------------------------------------------------------- */            
  { pR lc = &rinfo_[iinfo_[DG_IA_lc]];

    if (iinfo_[DG_I_TETA_A_NBASIS]>0)
      {
	mkS_lagrange_localspl_tria(&s_teta_a_degree,
				   &lc[iinfo_[DG_IA_lc_elm_a]],
				   &numValues);
      }
    
    mkS_lagrange_localspl_tria	(&s_teta_u_degree,
				 &lc[iinfo_[DG_IA_lc_elm_u0]],
				 &numValues);
    
    mkS_lagrange_localspl_tria	(&s_teta_u_degree,
				 &lc[iinfo_[DG_IA_lc_elm_u1]],
				 &numValues);
    
    mkS_bmapping		(qface_n_[0],
				 &lc[iinfo_[DG_IA_lc_face]],
				 &numValues,
				 qface_p_);
    ...
    mkS_basis(mkS_b(&s_teta_u),
	      &npts_velocity,
	      &rinfo_[iinfo_[DG_RA_EVAL_TETA_U]],
	      &iinfo_[DG_I_TETA_U_NBASIS],
	      &rinfo_[iinfo_[DG_IA_lc_u]],
	      &iinfo_[DG_I_nTot],// DG_I_numValues			   			   
	      rwork_,
	      rwork_n_,
	      ferr_);  }
...
#if 0
  {
    I i,j;pR lc = &rinfo_[iinfo_[DG_IA_lc]];
    for (i=0;i<npts_velocity;++i)
      {
	printf("allo %e %e "ifmt"\n",lc[iinfo_[DG_IA_lc_face]+i],lc[iinfo_[DG_IA_lc_face]+numValues+i],iinfo_[DG_I_TETA_U_NBASIS]);
	for (j=0;j<iinfo_[DG_I_TETA_U_NBASIS];++j)
	  {
	    printf("%e\n",rinfo_[iinfo_[DG_RA_EVAL_TETA_U] + iinfo_[DG_I_TETA_U_NBASIS]*i+j]);
	  }
      }
    exit(1);}
#endif
  ...
  { pR bmatflux = &rinfo_[iinfo_[DG_RA_bmatx]];
...
#if 0
  I s_teta_a[4]      = {iinfo_[DG_I_TETA_A_FAMILY],0,iinfo_[DG_I_TETA_A_DEGREE],iinfo_[DG_I_TETA_A_NBASIS]};
  I s_teta_u[4]      = {iinfo_[DG_I_TETA_U_FAMILY],0,iinfo_[DG_I_TETA_U_DEGREE],iinfo_[DG_I_TETA_U_NBASIS]};
  I s_teta[4]        = {iinfo_[DG_I_TETA_FAMILY],0,iinfo_[DG_I_TETA_DEGREE],iinfo_[DG_I_TETA_NBASIS]};
  I s_test[4]        = {iinfo_[DG_I_TEST_FAMILY],0,iinfo_[DG_I_TEST_DEGREE],iinfo_[DG_I_TEST_NBASIS]};
  I s_trial[4]       = {iinfo_[DG_I_TRIAL_FAMILY],0,iinfo_[DG_I_TRIAL_DEGREE],iinfo_[DG_I_TRIAL_NBASIS]};
#endif
		  ...
		  pR			rwork_)
{
  ...
  const I qelm_n_	[1] 	= {iinfo_[DG_I_QELM_N]*iinfo_[DG_I_QELM_N]};
  const I qface_n_	[1] 	= {iinfo_[DG_I_QFACE_N]};
  ...
  {
    const I rwork_n = (rwork_n_[0]>3*iinfo_[DG_I_QELM_N])?rwork_n_[0]-3*iinfo_[DG_I_QELM_N]:(I)0;
    if (rwork_n<1)
      {
    ...
    mkQ_legendre_interval_(&iinfo_[DG_I_QELM_N],
			   rwork_,
			   &negal1,
			   &rwork_[2*iinfo_[DG_I_QELM_N]],
			   &negal1,
			   &rwork_[3*iinfo_[DG_I_QELM_N]],
			   rwork_n_,
			   ferr_);    
    if (ferr_[0])
      {
    ...
    I qelm_n=0;
    mkQ_collapse_		(&iinfo_[DG_I_QELM_N],
				 rwork_,
				 &negal1,
				 &rwork_[2*iinfo_[DG_I_QELM_N]],
				 &negal1,
...
{  
 ...
 {I j;for (j=0;j<qface_n[0];++j)uface2[j] = (uface2[j]>0.0) ? uface2[j]*(xu_[0]) : (R)0.0;}
  ...
  /*--- COMPUTE MATRIX  */
  matrix_handle_gemv(&hbmat,"N",&r1,&hbrhs,&r0,&hlcmat);
	...
	for (int jadj=0;jadj<3;++jadj)
	  {
		...
		const I bmat_n 		= dg_iinfo[DG_I_bmat_n];
		const I bmat_m 		= dg_iinfo[DG_I_bmat_m];  
		// cst_pR eval_u_ 		= &dg_rinfo[dg_iinfo[DG_RA_EVAL_TETA_U]];
		// const I eval_uoff_      = dg_iinfo[DG_I_TETA_U_NBASIS];
		cst_pR bmat 	 	= &dg_rinfo[dg_iinfo[DG_RA_bmat]];
		cst_pR bmatflux 	= &dg_rinfo[dg_iinfo[DG_RA_bmatx]];
...
#if 0		
		{
		  I qface_n 	= dg_iinfo[DG_I_QFACE_N];
		  pR lc 	= &dg_rinfo[dg_iinfo[DG_IA_lc]];
		  pR lc_face 	= &lc[dg_iinfo[DG_IA_lc_face]];
