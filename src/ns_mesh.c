...
#ifndef NDEBUG
eDim		ns_mesh_dimension	(const ns_mesh*		self_){return self_->dimension;}
L 		ns_mesh_istransient	(const 	ns_mesh*self_) 	
{ 
  DebugVerif(self_); 
  return ( (self_->time_x) ? __emnsYES : __emnsNO ); 
}

eFace		ns_mesh_elm		(const 	ns_mesh*self_)	{ DebugVerif(self_); return self_->elm; }
I 		ns_mesh_nelm		(const 	ns_mesh*self_)	{ DebugVerif(self_); return self_->nelm; }
I 		ns_mesh_get_numEntities	(const ns_mesh*		self_,
					 const eTopologyDimension topologyDimension_)
{
  DebugVerif(self_);
  return self_->m_numEntities[topologyDimension_];
}

I 		ns_mesh_get_numNodes	(const 	ns_mesh*self_)	{ return ns_mesh_get_numEntities(self_,__eTopologyDimension_Point); }
I 		ns_mesh_get_numEdges	(const 	ns_mesh*self_)	{ return ns_mesh_get_numEntities(self_,__eTopologyDimension_Edge); }
I 		ns_mesh_nedge_boundary	(const 	ns_mesh*self_)	{ DebugVerif(self_); return self_->nedge_boundary; }

cst_pR 		ns_mesh_trelm		(const 	ns_mesh*self_)	{ DebugVerif(self_); return self_->trelm; }
cst_pR 		ns_mesh_jacelm		(const 	ns_mesh*self_)	{ DebugVerif(self_); return self_->jacelm; }
I 		ns_mesh_nddlspace	(const 	ns_mesh * 	self_,
					 const ensBASIS 	shape_)
{
  DebugVerif(self_);
  return self_->nddl_lagr[shape_];
}
...
void  ns_mesh_get_coovertex(const ns_mesh*	self_,
			    cst_pI 		vertexIndex_,
			    pR 			vertexCoo_)
{
#ifndef NDEBUG
  DebugVerif(self_);
  DebugVerif(vertexIndex_);
  DebugVerif(vertexCoo_);
#endif
  const I vertexIndex 	= vertexIndex_[0];
  const I dimension 	= ns_mesh_dimension(self_);
  { I j;
    for (j=0;j<dimension;++j)
      {
	vertexCoo_[j] = self_->coo[dimension*vertexIndex+j];
      } }
}



void ns_mesh_cncelm(const ns_mesh*	self_,
		    cst_pI 		cellIndex_,
		    pI         		cellToNodes_)
{
#ifndef NDEBUG
  DebugVerif(self_);
  DebugVerif(cellIndex_);
  DebugVerif(cellToNodes_);
  DebugVerif(cellIndex_[0]+1>0);
  DebugVerif(cellIndex_[0]<self_->nelm);
#endif
  cellToNodes_[0] = self_->cnc[cellIndex_[0]*6+0]-1;
  cellToNodes_[1] = self_->cnc[cellIndex_[0]*6+1]-1;
  cellToNodes_[2] = self_->cnc[cellIndex_[0]*6+2]-1;
  cellToNodes_[3] = self_->cnc[cellIndex_[0]*6+3]-1;
  cellToNodes_[4] = self_->cnc[cellIndex_[0]*6+4]-1;
  cellToNodes_[5] = self_->cnc[cellIndex_[0]*6+5]-1;
}



void ns_mesh_get_cellToNodes(const ns_mesh*	self_,
			     cst_pI 		cellIndex_,
			     pI         	cellToNodes_)
{
#ifndef NDEBUG
...
void ns_mesh_get_cellToEdges(const ns_mesh*	self_,
			     cst_pI 		cellIndex_,
			     pI         	cellToEdges_)
{
#ifndef NDEBUG
  DebugVerif(self_);
  DebugVerif(cellIndex_);
  DebugVerif(cellToEdges_);
  DebugVerif(cellIndex_[0]+1>0);
  DebugVerif(cellIndex_[0]<self_->nelm);
#endif
  const I n = ns_mesh_get_numNodes(self_)+1;
  cellToEdges_[0] = self_->cnc[cellIndex_[0]*6+3]-n;
  cellToEdges_[1] = self_->cnc[cellIndex_[0]*6+4]-n;
  cellToEdges_[2] = self_->cnc[cellIndex_[0]*6+5]-n;
  //  cellToEdges_[0] = self_->cnc[cellIndex_[0]*6+0]-n;
  //  cellToEdges_[1] = self_->cnc[cellIndex_[0]*6+1]-n;
  //  cellToEdges_[2] = self_->cnc[cellIndex_[0]*6+2]-n;
}
			  ...
			  pR         		cooelms_)
{
  ...
  { I i;
    for (i=0;i<nelm_[0];++i)
      {
	ns_mesh_get_cellToNodes(mesh_,elm_+i,cncelm);
		    ...
		    pR         	cooelm_)
{
  ...
  ns_mesh_get_cellToNodes(mesh_,ielm_,cncelm);
...
void ns_mesh_get_ddlcnc(const ns_mesh*	mesh_,
			...
			const I 	dec)
{
  const I numNodes = ns_mesh_get_numNodes(mesh_);
  const I numEdges = ns_mesh_get_numEdges(mesh_);
  ...
  if (NOT is_discont_)
	...
	{ 
	  ...
	  ddl_[4] = dec +mesh_->cnc[ielm_*6+4]-1;
	  ddl_[5] = dec +mesh_->cnc[ielm_*6+5]-1;
	  ddl_[6] = dec +numNodes+numEdges+ielm_;
	}
      else if (n_==10)
	{ 
	  ddl_[0] = dec +mesh_->cnc[ielm_*6+0]-1;
	  ...
	  ddl_[9] = dec +numNodes+2*numEdges+ielm_;
	}
      else if (n_==15)
	{ 
	  ddl_[0] = dec +mesh_->cnc[ielm_*6+0]-1;
	  ...
	  ddl_[12] = dec +numNodes+3*numEdges+ielm_;
	  ddl_[13] = dec +numNodes+3*numEdges+ielm_+1;
	  ddl_[14] = dec +numNodes+3*numEdges+ielm_+2;
	}
      else if (n_==21)
	{ 
	  ddl_[0] = dec +mesh_->cnc[ielm_*6+0]-1;
	  ...
	  ddl_[15] = dec +numNodes+4*numEdges+ielm_;
	  ddl_[16] = dec +numNodes+4*numEdges+ielm_+1;
	  ddl_[17] = dec +numNodes+4*numEdges+ielm_+2;
	  ddl_[18] = dec +numNodes+4*numEdges+ielm_+3;
	  ddl_[19] = dec +numNodes+4*numEdges+ielm_+4;
	  ddl_[20] = dec +numNodes+4*numEdges+ielm_+5;
	}
      else if (n_==28)
	{ 
	  ddl_[0] = dec +mesh_->cnc[ielm_*6+0]-1;
	  ...
	  ddl_[18] = dec +numNodes+4*numEdges+ielm_;
	  ddl_[19] = dec +numNodes+4*numEdges+ielm_+1;
	  ddl_[20] = dec +numNodes+4*numEdges+ielm_+2;
	  ddl_[21] = dec +numNodes+4*numEdges+ielm_+3;
	  ddl_[22] = dec +numNodes+4*numEdges+ielm_+4;
	  ddl_[23] = dec +numNodes+4*numEdges+ielm_+5;
	  ddl_[24] = dec +numNodes+4*numEdges+ielm_+6;
	  ddl_[25] = dec +numNodes+4*numEdges+ielm_+7;
	  ddl_[26] = dec +numNodes+4*numEdges+ielm_+8;
	  ddl_[27] = dec +numNodes+4*numEdges+ielm_+9;
	}
#if 0
      else
	{
...
void ns_mesh_edgejac(const ns_mesh*	mesh_,cst_pI iedge_,pR len_)
{
...
#endif
  const I numNodes = ns_mesh_get_numNodes(mesh_);
  const I ielm_[1] = {mesh_->patch[mesh_->bpatch[numNodes+iedge_[0]]]/*-1*/};
  if (mesh_->cnc[ielm_[0]*6+3+0]==numNodes+iedge_[0]+1)
    {
  ...
  if (mesh_->cnc[ielm_[0]*6+3+1]==numNodes+iedge_[0]+1)
    {
  ...
  if (mesh_->cnc[ielm_[0]*6+3+2]==numNodes+iedge_[0]+1)
    {
		    ...
		    pR 		normal_)
{
  ...
  I k;
  for (k=0;k<3;++k)
    if (mesh_->cnc[ielm_[0]*6+3+k]==numNodes+iedge_[0]+1)
      break;
#ifndef NDEBUG
  if (k>=3)
    {
		  ...
		  pI           jlocedge_)
{
  ...
  nadj_[0] 	= mesh_->bpatch[numNodes+iedge_[0]+1]-mesh_->bpatch[numNodes+iedge_[0]];
  ielm_[0] 	= mesh_->patch[mesh_->bpatch[numNodes+iedge_[0]]]-1;
  ilocedge_[0] 	= 0;
  if (mesh_->cnc[ielm_[0]*6+3+0]==numNodes+iedge_[0]+1)
    ilocedge_[0] = 0;
  else if (mesh_->cnc[ielm_[0]*6+3+1]==numNodes+iedge_[0]+1)
    ilocedge_[0] = 1;
  else if (mesh_->cnc[ielm_[0]*6+3+2]==numNodes+iedge_[0]+1)
    ilocedge_[0] = 2;
  if (nadj_[0]==2)
    {
      jelm_[0] = mesh_->patch[mesh_->bpatch[numNodes+iedge_[0]]+1]-1;
      if (mesh_->cnc[jelm_[0]*6+3+0]==numNodes+iedge_[0]+1)
	jlocedge_[0] = 0;
      else if (mesh_->cnc[jelm_[0]*6+3+1]==numNodes+iedge_[0]+1)
	jlocedge_[0] = 1;
      else if (mesh_->cnc[jelm_[0]*6+3+2]==numNodes+iedge_[0]+1)
	jlocedge_[0] = 2;
    } 
  else if (nadj_[0]!=1)
    {
		  ...
		  pI           jlocedge_)
{
  ...
  nadj_[0] 	= mesh_->bpatch[numNodes+iedge_[0]+1]-mesh_->bpatch[numNodes+iedge_[0]];
  ielm_[0] 	= mesh_->patch[mesh_->bpatch[numNodes+iedge_[0]]];
  ilocedge_[0] 	= 0;
  if (mesh_->cnc[ielm_[0]*6+3+0]==numNodes+iedge_[0]+1)
    ilocedge_[0] = 0;
  else if (mesh_->cnc[ielm_[0]*6+3+1]==numNodes+iedge_[0]+1)
  ...
  if (nadj_[0]==2)
    {
      jelm_[0] = mesh_->patch[mesh_->bpatch[numNodes+iedge_[0]]+1];
      if (mesh_->cnc[jelm_[0]*6+3+0]==numNodes+iedge_[0]+1)
	jlocedge_[0] = 0;
      else if (mesh_->cnc[jelm_[0]*6+3+1]==numNodes+iedge_[0]+1)
	jlocedge_[0] = 1;
...
{  
  ...
  if (!medit_reader)
    {
      sprintf(errmsg_,"ns_mesh_read:can not read '%s'",name);
      err_[0] =  __eErr_file;
      return;
    }
  ...
  if (((I)2)!=medit_dimension)
    {
      sprintf(errmsg_,"ns_mesh_read:expect dimension 2");
      err_[0] =  __eErr_file;
      return;
    }
  memset(mesh_,0,sizeof(ns_mesh));
  mesh_->dimension	=  __eDim_2;
  mesh_->m_numEntities[0]	=  Medit_get_nbVertices(medit_reader);
  mesh_->nelm  		=  Medit_get_nbFaces(medit_reader,__eFace_TRIANGLE);  
  mesh_->elm 		=  __eFace_TRIANGLE;
  ...
  mesh_->own_cnc  	= (pI)calloc(mesh_->nelm*((I)6),sizeof(I));
  mesh_->own_adj  	= (pI)malloc(sizeof(I)*mesh_->nelm*((I)3));
  ...
  /*\brief connectivity*/
  {
    I N = ns_mesh_get_numNodes(mesh_);
    I i;
    for (i=0;i<mesh_->nelm;++i)
      {
	{ I j;
    ...
    mesh_->m_numEntities[1] = N-mesh_->m_numEntities[0];  }
  ...
  cst_eFace elm = __eFace_TRIANGLE;
  const I numNodes = ns_mesh_get_numNodes(mesh_);
  const I numEdges = ns_mesh_get_numEdges(mesh_);

  mesh_->nddl_lagr[__ensBASIS_ERROR] 		= (I)0;
  mesh_->nddl_lagr[__ensBASIS_LAGRANGE_0] 	= mesh_->nelm;
  mesh_->nddl_lagr[__ensBASIS_LAGRANGE_1] 	= numNodes;
  mesh_->nddl_lagr[__ensBASIS_LAGRANGE_2] 	= numNodes+numEdges;
  mesh_->nddl_lagr[__ensBASIS_LAGRANGE_3] 	= numNodes+numEdges*2+mesh_->nelm;
  mesh_->nddl_lagr[__ensBASIS_LAGRANGEBUBBLE_0]	= 2*mesh_->nelm;
  mesh_->nddl_lagr[__ensBASIS_LAGRANGEBUBBLE_1]	= numNodes+mesh_->nelm;
  mesh_->nddl_lagr[__ensBASIS_LAGRANGEBUBBLE_2]	= numNodes+numEdges + mesh_->nelm;
  ...
  mesh_->own_coo 			= (pR)malloc(sizeof(R)*2*mesh_->nddlP6);
  mesh_->own_cod 			= (pI)malloc(sizeof(I)*mesh_->nddlP6);
  ...
  mesh_->own_jacelm	= (pR)malloc(sizeof(R)*mesh_->nelm);
  mesh_->own_trelm	= (pR)malloc(sizeof(R)*mesh_->nelm*4);
  ...
  mesh_->own_bpatch		= (pI)calloc(mesh_->nddlP6+1,sizeof(I));
  mesh_->own_patch		= (pI)malloc(sizeof(I)*mesh_->nelm*6);
  ...
  //  printf("######################### numEdges "ifmt"\n",numEdges);
  mesh_->own_jacedge = (pR)malloc(sizeof(R)*numEdges);
  ...
  mesh_->own_normaledge = (pR)malloc(sizeof(R)*numEdges*2);
  ...
  */
  { 
	...
	mesh_->time_x = (pR)malloc(sizeof(R)*mesh_->time_n);
	{ I i;
	  for (i=0;i<mesh_->time_n;++i)
	    {
...
{  
  ...
  mesh_->coo 		= (pR)malloc(sizeof(R)*medit_dim*mesh_->nddlP6);
  mesh_->cod 		= (pI)malloc(sizeof(I)*mesh_->nddlP6);
  mesh_->m_numEntities[1] = N-numNodes;  
  mesh_->nedge_boundary = 0;    

  const I numEdges = ns_mesh_get_numEdges
    (mesh_);

  mesh_->nddl_lagr[__ens_lagr_tria_error] = (I)0;
  mesh_->nddl_lagr[__ens_lagr_tria_01] 	= mesh_->nelm;
  mesh_->nddl_lagr[__ens_lagr_tria_03] 	= numNodes;
  mesh_->nddl_lagr[__ens_lagr_tria_06] 	= numNodes+numEdges;
  mesh_->nddl_lagr[__ens_lagr_tria_07] 	= numNodes+numEdges + mesh_->nelm;
  mesh_->nddl_lagr[__ens_lagr_tria_10] 	= numNodes+numEdges*2+mesh_->nelm;
  mesh_->nddl_lagr[__ens_lagr_tria_15] 	= numNodes+numEdges*3+mesh_->nelm*3;
  mesh_->nddl_lagr[__ens_lagr_tria_21] 	= numNodes+numEdges*4+mesh_->nelm*6;
  mesh_->nddl_lagr[__ens_lagr_tria_28]	= numNodes+numEdges*5+mesh_->nelm*10;
  mesh_->nddlP6 			= mesh_->nddl_lagr[__ens_lagr_tria_06];
  mesh_->jacelm		= (pR)malloc(sizeof(R)*mesh_->nelm);
  mesh_->trelm		= (pR)malloc(sizeof(R)*mesh_->nelm*4);
  { I i;
    R tmpcooelm[6];
    for (i=0;i<mesh_->nelm;++i)
      {
  ...
  printf("######################### numEdges "ifmt"\n",numEdges);
  mesh_->jacedge = (pR)malloc(sizeof(R)*numEdges);
  {
    I iedge;
    for (iedge = 0;iedge<numEdges;++iedge)
      {
  ...
  mesh_->normaledge = (pR)malloc(sizeof(R)*numEdges*2);
  {
    /*
      on calcule la normale vers l'exterieur du premier element du patch de l'arete
    */
...
void ns_mesh_set_periodic(ns_mesh*mesh_)  
{
  ...
  I * jd14,*jd23;
  id14 = (pI)malloc(sizeof(I)*(count14+1));
  id23 = (pI)malloc(sizeof(I)*(count23+1));
  jd14 = (pI)malloc(sizeof(I)*(count14+1));
  jd23 = (pI)malloc(sizeof(I)*(count23+1));
#if 0
...
void ns_mesh_write_medit_with_codelm(const ns_mesh*s_,cst_pI codelm_,const char * name_,...)
{
  { char ctmp[512];
    ...
    fprintf(fil,"MeshVersionFormatted\n1\nDimension\n2\nVertices\n"ifmt"\n",numNodes);
    {I i;for (i=0;i<numNodes;++i){ fprintf(fil,""rfmt" "rfmt" "ifmt"\n",s_->coo[i*2+0],s_->coo[i*2+1],s_->cod[i]); } }
    fprintf(fil,"Triangles\n"ifmt"\n",s_->nelm); 
    I cncelm[3];
    { I i;
      for (i=0;i<s_->nelm;++i)
...
{  
  ...
  { I i;
	...
	if (y<value_axe+1.0e-5)
	  {
	    perm_[i]=numNodes-1-nvertex_axi_[0];
	    nvertex_axi_[0]++;
	  }
	else
	  {
			      ...
			      cst_pR                   vectory_) 
{
  ...
  const I numNodes = ns_mesh_get_numNodes(mesh_);
  { I i;
    for (i=nvertex_noaxi_[0];i<numNodes;++i)
      {
	if (axe_[0]==0)
	  {
	    fprintf(out,"" rfmt " ",vectory_[rperm_[i]]*1.0);  		
...
{  
  ...
  for (i=nvertex_noaxi;i<numNodes;++i)
    {      
      I p = rperm_[i];
      if (axe_[0]==1)
	{
  ...
  for (irot=1;irot<=nrot_[0];++irot)
	...
	{      
	  jrot	= 1;
	  jrot2 = 1;
	  dec   = numNodes+(irot-2)*nvertex_noaxi;
	  dec2  = 0;      
	}
      else if (irot==1)
	{
      ...
      else
	{
	  ...
	  jrot2 = 1;
	  dec   = numNodes+(irot-2)*nvertex_noaxi;
	  dec2  = numNodes+(irot-1)*nvertex_noaxi;      
	}
      {
	for (i=0;i<mesh_->nelm;++i)
	  {
...
{  
  ...
  const I numNodes = ns_mesh_get_numNodes(mesh_);
  const I numEdges = ns_mesh_get_numEdges(mesh_);
  pI ddlcnc2 = (pI)calloc(nelm2*3,sizeof(I));
  {
  ...
  for (i=nvertex_noaxi;i<numNodes+numEdges;++i)
    {      
      I p = rperm_[i];
      if (axe_[0]==1)
	{
  ...
  for (irot=1;irot<=nrot_[0];++irot)
	...
	{      
	  jrot	= 1;
	  jrot2 = 1;
	  dec   = numNodes+numEdges+(irot-2)*nvertex_noaxi;
	  dec2  = 0;      
	}
      else if (irot==1)
	{
	  ...
	  jrot2 = 1;
	  dec   = 0;      
	  dec2  = numEdges+numNodes;
	}
      else
	{
	  jrot	= 1;
	  jrot2 = 1;
	  dec   = numEdges+numNodes+(irot-2)*nvertex_noaxi;
	  dec2  = numEdges+numNodes+(irot-1)*nvertex_noaxi;      
	}
      {
	for (i=0;i<nelm2;++i)
	  {
...
{
  integrator_adt2d P = (integrator_adt2d)malloc(sizeof(integrator_adt2d_st));
  I idim;
  P->racine = NULL;
...
void * ns_mesh_localization_kill(void * localization_)
{
  if (localization_)
    {
      integrator_adt2d_kill((integrator_adt2d)localization_);
    }
  return NULL;
}
...
void* ns_mesh_localization(const ns_mesh*mesh_)
{
  ...
  { I i;
	...
	integrator_adt2d_insert((integrator_adt2d)localization_data,p0,p1,i);
      } }
  ...
  while (select_n_[0]==0)
    {
      ...
      integrator_adt2d_select((integrator_adt2d)localize,&cc1[0],&cc2[0],select_,select_n_);
#ifndef NDEBUG
#if 0
      if (NOT select_n_[0])
	{
