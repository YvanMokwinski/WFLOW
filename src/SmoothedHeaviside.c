...
void SmoothedHeaviside_def(pSmoothedHeaviside 	const 	self_,
			   cst_eHeaviside		heaviside_,
			   cst_pR 		const 	eps_,
			   pErr			const 	err_) 
{
  err_[0] = __eErr_no;
  memset(self_,0,sizeof(SmoothedHeaviside));
  switch(heaviside_)
    {
    case __eHeaviside_m0p2:
    case __eHeaviside_m0p3:
    case __eHeaviside_m0p4:
    case __eHeaviside_m0p5:
    case __eHeaviside_m0p6:
    case __eHeaviside_m2p0:
    case __eHeaviside_m2p1:
    case __eHeaviside_m2p2:
    case __eHeaviside_m2p3:
    case __eHeaviside_m4p0:
    case __eHeaviside_m4p1:
    case __eHeaviside_m4p2:
    case __eHeaviside_m4p3:
    case __eHeaviside_sin:
      {
	self_->m_heaviside	= heaviside_;
	self_->m_eps		= eps_;
	break;
      }

    case __eHeaviside_ERROR:
    case __eHeaviside_ALL:
      {
...
Err ns_heaviside(cst_eHeaviside kind_heaviside_,
		 const I 	rn_,
		 pR 		const r_,
		 cst_pR 	const eps_)
{
  Err err = __eErr_no;
  const R ieps = ((R)1.0)/eps_[0];
  switch(kind_heaviside_)
    {
    case __eHeaviside_m0p2:
    case __eHeaviside_m0p3:
    case __eHeaviside_m0p4:
	...
	{ I _jj;
	  for (_jj=0;_jj<rn_;++_jj)
	      ...
	      else
		{
		  R a,b;
		  const R t2 = t*ieps;
		  horner_a(kind_heaviside_,a,t2);
		  horner_b(kind_heaviside_,b,t2);
		  r_[_jj] = a*b;
		}
	    } }
	break;
	...
	{ I _jj;
	  for (_jj=0;_jj<rn_;++_jj)
	      ...
	      else
		{
		  ...
		  r_[_jj] = (r1_2+t*ieps*((R)0.5)+MNS_TWICEPIINVERSE*nsSIN(pidiveps*t)); 
		}
	    } }
	break;
      }
				 ...
				 pErr					err_)
{
  ...
  err_[0]= ns_heaviside(self_->m_heaviside,
			rn_,
			r_,
			self_->m_eps);
}
